...
static datum
gc(int alen, int slen, int blen, int c, ...)
{
    int i, live = 0;
    pair np;
    free_index = 0;
...
cons(datum x, datum y)
{
    ...
    if ((free_index + 3) >= HEAP_SIZE) gc(-1, -1, -1, 2, &x, &y);
...
make_array(uint len)
{
    ...
    if ((free_index + (len + 1)) >= HEAP_SIZE) return gc(len, -1, -1, 0);
    p = &busy_pairs[free_index++];
...
make_bytes(uint len)
{
    ...
    if ((free_index + (words + 1)) >= HEAP_SIZE) return gc(-1, len, -1, 0);
...
make_obj(uint len)
{
    ...
    if ((free_index + (len + 1)) >= HEAP_SIZE) return gc(-1, -1, len, 0);
